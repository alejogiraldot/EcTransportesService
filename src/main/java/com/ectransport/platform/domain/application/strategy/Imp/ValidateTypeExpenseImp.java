...
@Component
public class ValidateTypeExpenseImp extends AbstractValidateTypeUpload<FileUploadStrategyDto> {
  private final UploadDocumentService uploadDocumentService;
  private final UploadFileRepository uploadFileRepository;
  private final UploadApplicationMapper uploadApplicationMapper;

  public ValidateTypeExpenseImp(UploadDocumentService uploadDocumentService, UploadFileRepository uploadFileRepository, UploadApplicationMapper uploadApplicationMapper) {
  ...
  public List<FileUploadResponseDto> validateTypeUpload(FileUploadStrategyDto parameter) {
    return Optional.ofNullable(parameter.getExpenseDataUploadDtoList())
        .filter(list -> !list.isEmpty())
        .map(list -> list.stream()
            .map(expenseDataUploadDto -> handleUploadData(
                parameter.getIdentification(),
                expenseDataUploadDto,
  ...
  private FileUploadResponseDto handleUploadData(
      ...
      ExpenseDataUploadDto uploadDataDto,
      UUID fkService
  ) {
    try {
  ...
  private void setValueUploadData(ExpenseDataUploadDto uploadDataDto, MediaManagerResponseDto mediaManagerResponseDto, UUID fkService) {
    mediaManagerResponseDto.setFkService(fkService);
    mediaManagerResponseDto.setAmount(uploadDataDto.getAmount());
    mediaManagerResponseDto.setPaymentType(uploadDataDto.getExpense());
    mediaManagerResponseDto.setFkTypeUpload(UploadConstant.EXPENSE_KEY);
  }
  ...
  private MediaManagerResponseDto setDataWithoutUpload(ExpenseDataUploadDto uploadDataDto, UUID fkService) {
    return MediaManagerResponseDto.builder()
        ...
        .paymentType(uploadDataDto.getExpense())
        .fileName(null)
        .fileSize(null)
        .uploadDate(LocalDateTime.now())
        .fkTypeUpload(UploadConstant.EXPENSE_KEY)
        .build();
  }
}
